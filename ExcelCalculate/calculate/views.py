...
def calculate(request):
    ...
    for key in grade_dic.keys():
        ...
        grade_calculate_dic[key]["avg"] = float(sum(grade_dic[key])) / len(
            grade_dic[key]
        )
    # 결과 출력
    ...
    for key in grade_list:
        ...
        grade_calculate_dic_to_session[int(key)]["max"] = float(
            grade_calculate_dic[key]["max"]
        )
        grade_calculate_dic_to_session[int(key)]["avg"] = float(
            grade_calculate_dic[key]["avg"]
        )
        grade_calculate_dic_to_session[int(key)]["min"] = float(
            grade_calculate_dic[key]["min"]
        )
